...
check_ver() {
	...
	if [ "$SUPPORTED_OS_VERSIONS" == "22.04" ]; then
		nodejs_ver=v22.12.0
		neovim_ver=0.10.4
		apt_package="gcc g++ curl wget git ripgrep build-essential make cmake python3.10 libpython3-dev python3-distutils python3-pip clang clang-tidy clang-format universal-ctags bear libtool-bin automake autoconf bison gperf flex texinfo libncurses5 libncurses5-dev ninja-build gettext unzip pkg-config rename software-properties-common npm xclip fd-find lua-5.4 luarocks"
		apt_arr=("$apt_package")
	elif [ "$SUPPORTED_OS_VERSIONS" == "20.04" ]; then
		nodejs_ver=v20.18.1
		neovim_ver=0.10.4
		apt_package="gcc g++ curl wget git ripgrep build-essential make cmake python3.8 libpython3-dev python3-distutils python3-pip clang clang-tidy clang-format universal-ctags bear libtool-bin automake autoconf bison gperf flex texinfo libncurses5 libncurses5-dev ninja-build gettext unzip pkg-config rename software-properties-common npm xclip fd-find lua-5.4 luarocks"
		apt_arr=("$apt_package")
	else
...
install_nodejs() {
	...
	cd ./$PACKAGE_DIR/node-$nodejs_ver || exit 1 
	./configure
	make -j4
